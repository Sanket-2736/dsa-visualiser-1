...
const bubbleSortSteps = (array) => {
  ...
  for (let i = 0; i < N; i++) {
    ...
    for (let j = 0; j < N - i - 1; j++) {
      const step = { arr: [...a], compare: [j, j + 1], sorted: Array.from({ length: N }, (_, k) => k >= N - i) };
      ...
      if (a[j].value > a[j + 1].value) {
        ...
        steps.push({ arr: [...a], swap: [j, j + 1], sorted: Array.from({ length: N }, (_, k) => k >= N - i) });
      }
    }
    
    if (i < N - 1) {
        steps.push({ arr: [...a], sorted: Array.from({ length: N }, (_, k) => k >= N - i - 1) });
    }

    if (!swapped) break;
  }
  
  steps.push({ arr: [...a], sorted: Array(N).fill(true) });
...
const BubbleSortPage = () => {
  ...
  const highlight = currentStepData.compare || [];
  ...
  const sortedArrForMotion = useMemo(() => {
    return [...currentArr].sort((a, b) => a.id - b.id);
  }, [currentArr]);
  ...
  const statusMessage = useMemo(() => {
      if (isFinished) {
          return "Sorting complete! Array is fully sorted.";
      }
      if (swap.length > 0) {
          return `Swap: ${currentArr[swap[0]].value} and ${currentArr[swap[1]].value}`;
      }
      if (highlight.length > 0) {
          return `Comparing: ${currentArr[highlight[0]].value} and ${currentArr[highlight[1]].value}`;
      }
      return "Ready to start visualization.";
  }, [isFinished, swap, highlight, currentArr]);
      ...
      <div className="max-w-4xl mx-auto text-center">
        ...
        <div className="flex flex-col md:flex-row justify-center items-center gap-4 mb-10 bg-gray-800/50 p-4 rounded-lg shadow-xl">
            ...
            <div className="flex flex-col items-center">
                ...
                <input
                    ...
                    onBlur={reset}
                    className="bg-gray-900 border border-indigo-500 text-white px-2 py-1 rounded-md w-20 text-center"
                />
            </div>
          ...
          {currentArr.map((item, index) => (
            <motion.div
              ...
              className={`flex-shrink-0 w-6 mx-0.5 rounded-t-sm flex justify-center items-end text-[0.6rem] font-bold text-gray-900 
                ${sorted[index] ? "bg-green-500" : 
                 (swap.includes(index) ? "bg-red-500" :
                 (highlight.includes(index) ? "bg-yellow-400" : "bg-indigo-500"))
               }`}
        ...
        <div className="mt-8 flex justify-between items-center max-w-3xl mx-auto">
            <p className="text-sm text-indigo-300">
                Step <span className="font-bold">{currentStep}</span> / {steps.length}
            </p>
            <p className="text-sm text-indigo-300">
                <span className="font-bold">N:</span> {size} | <span className="font-bold">Max Value:</span> 99
            </p>
        </div>
      </div>
    </div>
